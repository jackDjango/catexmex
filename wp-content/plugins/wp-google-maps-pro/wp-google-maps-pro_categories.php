...
function wpgmza_pro_category_new_layout() {
    ...
    foreach ($map_ids as $map_id) {
        ...
        echo "                  <div class='switch'><input class='cmn-toggle cmn-toggle-round-flat' type='checkbox' id='map-cat-".$map_id."' name='assigned_to_map[]' value='".$map_id."'> <label for='map-cat-".$map_id."'></label></div>".$map_data->map_title."  (id ".$map_id.")<br />";
    }
    echo "              </td>";
    echo "          </tr>";
...
function wpgmaps_category_head() {
    ...
    if (isset($_GET['page']) && $_GET['page'] == "wp-google-maps-menu-categories" && isset($_POST['wpgmza_save_marker_category'])) {
        if (isset($_POST['wpgmza_save_marker_category'])){
            ...
            if (!isset($_POST['assigned_to_map'])) { $_POST['assigned_to_map'][0] = "All"; }
...
function wpgmaps_list_categories() {
    ...
    echo "<table class=\"wp-list-table widefat fixed wpgmza-listing\" cellspacing=\"0\">";
    echo "  <thead>";
    echo "      <tr>";
